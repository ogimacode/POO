...
public class SQLiteProductDAO implements ProductDAO{
    ...
    public Optional<ProductDTO> findOne(String sku) {
        ...
        try (var statement = ConnectionFactory.getPreparedStatement(sql)) {
            ...
            if (rs.next())
                return Optional.of(new ProductDTO(rs.getString("code"), rs.getString("name"), rs.getInt("quantity"), rs.getDouble("price")));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static ProductDTO toDto(Product product) {
        return new ProductDTO(product.getSku(), product.getName(), product.getQuantity(), product.getPrice());
    }
    public static Product fromDto(ProductDTO dto){
        return new Product(dto.code(), dto.name(), dto.quantity(), dto.price());
    }
}
