...
public class EmployeeServices{
    ...
    public EmployeeServices(Map<String, Employee> employeeMap) {
    ...
    public void increaseSalary(String idEmployee, double newSalary){
            ...
            employeeMap.values().stream()
                .filter(employee -> employee.getId().equals(idEmployee)).findAny()
                    .ifPresentOrElse(
                            employee -> employee.setSalary(newSalary),
                            ()->{throw new IdNotBeNullExecption("The employee not exists");});
    ...
    public double averageSalary(LocalDate inicialDate, LocalDate finalDate){
        return employeeMap.values().stream()
                .filter(employee -> employee.getDateOfEmployment().isBefore(finalDate) &&
                        employee.getDateOfEmployment().isAfter(inicialDate))
                .mapToDouble(Employee::getSalary).average().orElse(0);
    }
